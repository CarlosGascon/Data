...
- When center values are given (e.g., in Numerical Simulations), the corresponding matrix accounts for the results obtained exactly at the grid points defined by acenters and ecenters (or mcenters). Therefore, if matrix is Nm x Na, then acenters and mcenters have Na and Nm points, respectively.
- When edge values are given (e.g., in Systems Prioritization), the corresponding matrix accounts for the results obtained as the average of the values at the edges of each bin defined by aedges and Redges. Therefore, if matrix is Nr x Na, then aedges and Redges have Na + 1 and Nr + 1 points, respectively.
